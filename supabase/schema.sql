...
-- ============================================
-- External Deals (aggregated from 3rd-party sites)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_external_deals_status ON external_deals(status);
CREATE INDEX IF NOT EXISTS idx_external_deals_posted ON external_deals(posted_at DESC);
CREATE INDEX IF NOT EXISTS idx_external_deals_source ON external_deals(source);

ALTER TABLE external_deals ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Public read external_deals" ON external_deals FOR SELECT USING (true);

-- Enable Realtime for live deal updates
ALTER PUBLICATION supabase_realtime ADD TABLE external_deals;

-- RPC: append a source to deal's sources array without duplicates
CREATE OR REPLACE FUNCTION append_deal_source(p_asin TEXT, p_source TEXT)
RETURNS void AS $$
BEGIN
